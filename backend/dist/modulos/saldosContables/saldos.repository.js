...
class SaldosRepository extends base_firebaseRepository_1.BaseFirebaseRepository {
     ...
     */
    extraerCodigosCuentasDeComponentes(componentes) {
     ...
     */
    async obtenerSaldosPorOficinaFechaYCuentas(codigoOficina, fechas, codigosCuentas) {
        try {
    ...
    async obtenerSaldosPorOficinaYFecha(codigoOficina, fechas) {
        try {
            // Convertir fechas a formato string YYYY-MM-DD
            const fechasFormateadas = fechas.map(f => f.toISOString().split('T')[0]);
