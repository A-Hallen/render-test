...
class ExportService {
     ...
     */
    async generarExcel(reporteData) {
        try {
            ...
            const headerStyle = {
                ...
                fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: '1E40AF' } }, // Azul corporativo
                border: {
            ...
            const categoryStyle = {
                font: { bold: true, color: { argb: 'FFFFFF' } },
                fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: '3B82F6' } }, // Azul más claro
                border: {
                    top: { style: 'thin', color: { argb: 'C7D2FE' } },
                    left: { style: 'thin', color: { argb: 'C7D2FE' } },
                    bottom: { style: 'thin', color: { argb: 'C7D2FE' } },
            ...
            const totalStyle = {
                font: { bold: true, color: { argb: 'FFFFFF' } },
                fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: '3B82F6' } }, // Azul más claro para mejor contraste
                border: {
                    top: { style: 'thin', color: { argb: 'C7D2FE' } },
                    left: { style: 'thin', color: { argb: 'C7D2FE' } },
                    bottom: { style: 'thin', color: { argb: 'C7D2FE' } },
            ...
            const rowStyle = {
                fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'EFF6FF' } }, // Azul muy claro
                border: {
                    top: { style: 'thin', color: { argb: 'C7D2FE' } },
                    left: { style: 'thin', color: { argb: 'C7D2FE' } },
                    bottom: { style: 'thin', color: { argb: 'C7D2FE' } },
            ...
            titleCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'EFF6FF' } };
            titleCell.border = {
                top: { style: 'thin', color: { argb: 'C7D2FE' } },
                left: { style: 'thin', color: { argb: 'C7D2FE' } },
                bottom: { style: 'thin', color: { argb: 'C7D2FE' } },
            ...
            periodoCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'EFF6FF' } };
            periodoCell.border = {
                top: { style: 'thin', color: { argb: 'C7D2FE' } },
                left: { style: 'thin', color: { argb: 'C7D2FE' } },
                bottom: { style: 'thin', color: { argb: 'C7D2FE' } },
            ...
            oficinaCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'EFF6FF' } };
            oficinaCell.border = {
                top: { style: 'thin', color: { argb: 'C7D2FE' } },
                left: { style: 'thin', color: { argb: 'C7D2FE' } },
                bottom: { style: 'thin', color: { argb: 'C7D2FE' } },
            ...
            for (const categoria of reporteData.categorias) {
                ...
                for (let i = 0; i < reporteData.fechas.length; i++) {
                    ...
                    if (i === 0)
                        firstValue = value;
                    if (i === reporteData.fechas.length - 1)
                        lastValue = value;
                    colIndex++;
                }
    ...
    async generarPDF(reporteData) {
        try {
            ...
            // Añadir título
            doc.fontSize(18).font('Helvetica-Bold').fillColor('#212529').text(`Reporte de Contabilidad - ${reporteData.nombreConfiguracion}`, { align: 'center' });
            // Añadir información del reporte
            doc.moveDown();
            doc.fontSize(12).font('Helvetica').text(reporteData.descripcionPeriodo || `Periodo: ${reporteData.fechaInicio} al ${reporteData.fechaFin}`, { align: 'center' });
            doc.moveDown(0.5);
            ...
            const getDiffColor = (value) => {
                if (value > 0)
                    return colors.positiveDiff;
                if (value < 0)
                    return colors.negativeDiff;
                return '#212529'; // Color negro por defecto
            };
            // Función para dibujar una celda de tabla
            const drawCell = (text, x, width, height, options = {}) => {
                const { isHeader = false, isCategory = false, isTotal = false, isAlternateRow = false, isNumeric = false, diffValue = 0, align = 'left' } = options;
                // Dibujar borde
            ...
            doc.moveDown(2);
            doc.fontSize(8).text(`Generado el ${new Date().toLocaleDateString('es-ES')} a las ${new Date().toLocaleTimeString('es-ES')}`, { align: 'right' });
            // Finalizar documento
            doc.end();
