...
class AuthController {
    ...
    async register(req, res) {
        try {
            const { email, password, displayName, role = user_interface_1.UserRole.USER, officeId } = req.body;
    ...
    async updateUserRole(req, res) {
        try {
            ...
            // Validar que se proporcione officeId cuando el rol es GERENTE_OFICINA o ANALISTA
            if ((newRole === user_interface_1.UserRole.GERENTE_OFICINA || newRole === user_interface_1.UserRole.ANALISTA) && !officeId) {
                res.status(400).json({
                    message: 'Se requiere ID de oficina para usuarios con rol de gerente o analista',
                    code: 'auth/missing-office-id'
                });
