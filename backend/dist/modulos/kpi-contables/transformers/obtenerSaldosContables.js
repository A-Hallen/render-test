...
exports.obtenerFechasEntrePeriodo = exports.obtenerSaldosContables = void 0;
...
const obtenerSaldosContables = async (oficina, inicio, fin) => {
    try {
        ...
        const fechas = (0, exports.obtenerFechasEntrePeriodo)(inicio, fin);
        ...
        try {
            const saldos = await saldosRepository.obtenerSaldosPorOficinaYFecha(oficina, fechas);
 ...
 */
function generarSaldosDePrueba(oficina, fechas) {
...
const obtenerFechasEntrePeriodo = (inicio, fin) => {
    ...
    let fechaActual = new Date(fechaInicio.getFullYear(), fechaInicio.getMonth() + 1, 0);
    while (fechaActual <= fechaFin) {
        fechas.push(new Date(fechaActual));
        // Avanzar al último día del siguiente mes
        fechaActual = new Date(fechaActual.getFullYear(), fechaActual.getMonth() + 2, 0);
    }
    console.log(`Se calcularán KPIs para ${fechas.length} meses: ${fechas
        .map((f) => f.toISOString().split("T")[0])
        .join(", ")}`);
    return fechas;
};
exports.obtenerFechasEntrePeriodo = obtenerFechasEntrePeriodo;
