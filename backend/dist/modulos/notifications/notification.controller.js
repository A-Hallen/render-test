...
class NotificationController {
     ...
     */
    async saveAnonymousToken(req, res) {
        try {
            const { token, deviceId } = req.body;
            // Validar datos de entrada
            if (!token || !deviceId) {
     ...
     */
    async sendNotificationToMultipleUsers(req, res) {
        try {
    ...
    async getUserNotifications(req, res) {
        try {
            ...
            if (req.user && req.user.uid !== userId && req.user.role !== user_interface_1.UserRole.ADMIN) {
