  ...
  try {
    ...
    try {
      ...
      if (codigosCuentasNecesarias.length > 0) {
        saldos = await saldosRepository.obtenerSaldosPorOficinaFechaYCuentas(oficina, fechaFinObj, fechaInicioObj, codigosCuentasNecesarias, fechaFinObj === fechaInicioObj);
      } else {
        saldos = await saldosRepository.obtenerSaldosPorOficinaYFecha(oficina, fechaFinObj, fechaInicioObj, true);
      }
    } catch (error) {
